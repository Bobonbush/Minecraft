...
void QuadRenderer::LoadData(const glm::vec2 &coords) {
    ...
    if(Mineral) {
        textureCoords = ItemDataBase::GetInstance() -> MineralAtlas.getTexture(coords);
    }
...
void QuadRenderer::add(const glm::vec3 & position) {
    quads.push_back(std::make_pair(position, Mineral));
}
...
void QuadRenderer::renderQuads(const glm::mat4 & view, const glm::mat4 &projection, const glm::vec3 & size) {
    ...
    for(auto & quad : quads) {
        ...
        model = glm::mat4(1.0f);
        model = glm::translate(model, quad.first);

...
void ChunkMesh::addFace(const std::vector<GLfloat> & vertices, const std::vector<GLfloat> & normals, const std::vector<GLfloat> &texcoords, const glm::vec3 &position, const glm::vec3 &blockPosition) {
    std::vector<GLfloat> &m_vertices = mesh.vertexPosition;
    std::vector<GLfloat> &m_coords = mesh.textureCoords;
    std::vector<GLuint> &m_indices = mesh.indices;
    std::vector<GLfloat> &m_normals = mesh.normals;
